...
def weater_scraping(url, areaflag):
...
if __name__ == "__main__":
    ...
    proxies = {"http": "http://{0}:{1}@proxy.wrc.melco.co.jp:9515/".format(ID, PASS),
               "https": "https://{0}:{1}@proxy.wrc.melco.co.jp:9515/".format(ID, PASS)}
        ...
        try:
            ...
            for i in range(testterm + 1):
                ...
                if i == 0:
                    ...
                    weater_scraping(url, areaflag).to_csv(
                        '気象庁データ_{0}_{1}_{2}{3}{4}.csv'.format(areacode1, areacode2, start.year, start.month,
                                                              start.day), mode='a', header=False,encoding="shift_jis")
            print("データの取得が完了しました")
